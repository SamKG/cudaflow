...
fn main() {
    ...
    create_cuda_driver_internal_bindings(&sdk, &outdir, &manifest_dir);
    ...
    create_cupti_bindings(&sdk, &outdir, &manifest_dir);
...
fn create_cuda_driver_internal_bindings(
    sdk: &cuda_sdk::CudaSdk,
    outdir: &path::Path,
    manifest_dir: &path::Path,
) {
    ...
    let bindgen_path = path::PathBuf::from(format!("{}/driver_internal_sys.rs", outdir.display()));
    let header = manifest_dir.join("build/driver_internal_wrapper.h");
    println!("cargo::rerun-if-changed={}", header.display());
    let bindings = bindgen::Builder::default()
        .header(header.to_str().expect("header should be valid UTF-8"))
        .parse_callbacks(Box::new(
...
fn create_cupti_bindings(sdk: &cuda_sdk::CudaSdk, outdir: &path::Path, manifest_dir: &path::Path) {
    ...
    let bindgen_path = path::PathBuf::from(format!("{}/cupti_sys.rs", outdir.display()));
    let header = manifest_dir.join("build/cupti_wrapper.h");
    println!("cargo::rerun-if-changed={}", header.display());
    let bindings = bindgen::Builder::default()
        .header(header.to_str().expect("header should be valid UTF-8"))
        .parse_callbacks(Box::new(
    ...
    bindings
        .write_to_file(bindgen_path.as_path())
        .expect("Cannot write CUPTI bindgen output to file.");
}
